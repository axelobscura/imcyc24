    ...
    return (
        <div className="grid grid-cols-[1fr] w-full rounded-md items-center">
            <div className="grid grid-cols-[1fr] sm:grid-cols-[1fr] gap-0 my-5">
                <h3 className="text-center text-1xl font-semibold text-white p-0 m-0 pb-2">EVALUACIÓN PARA</h3>
                <h2 className="text-center text-4xl font-bold text-white p-0 m-0 text-blue-500">{typeof params?.webinar === 'string' ? decodeURIComponent(params.webinar.split('-').join(' ')).toUpperCase() : ''}</h2>
            </div>
            <div className="grid grid-cols-[1fr] sm:grid-cols-[1fr] gap-0 my-2">
                <p className="text-left text-1xl text-white p-0 m-0 uppercase pl-6 flex items-center"><FaClock className="mr-2" /> Cuenta con <span className="font-bold mx-2 text-2xl text-blue-500">{countdown}</span> para terminar la evaluación:</p>
            </div>
