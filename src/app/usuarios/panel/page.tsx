          ...
          <div className="text-center">
            ...
            <div className="grid grid-cols-[1fr] sm:grid-cols-[1fr] gap-4">
              <div className="bg-opacity-75 shadow-lg border-blue-700">
                <div className='grid grid-cols-[1fr_1fr] gap-0 mt-4'>
                  <div className='p-3 bg-slate-850'>
                    <h3 className='text-left text-2xl text-white uppercase'>Contenido IMCYC</h3>
                  ...
                  <div className='p-3 border-slate-700 border-l-2'>
                    <h3 className='text-left text-2xl text-white uppercase'>Valor Agregado</h3>
                    <div className='grid grid-cols-[1fr_1fr_1fr] gap-4 mt-4 items-center'>
                      {categorias && categorias.map((categoria: { id: number; nombre: string; descripcion: string; imagen: string, id_tipo: number }) => {
                        if (categoria.id_tipo === 2) {
                          return (
                            <Link key={categoria.id} href={`/usuarios/concreton/${categoria.id}`} className='hover:scale-110 transition duration-500 ease-in-out hover:z-10 hover:shadow-lg hover:shadow-gray-900 hover:border-gray-900'>
                              <div className="mt-1 p-0 bg-gray-800 hover:bg-slate-300 bg-opacity-50 rounded-lg shadow-md text-left items-center justify-center w-full hover:text-gray-900">
                                <div className="w-full h-[140px] overflow-hidden  bg-[url('https://www.webinarsenconcreto.com/images/2022/fondo1.jpg')] bg-gray-700 bg-blend-multiply z-10 bg-cover bg-center bg-no-repeat">
                                  <Image src={`/webinars/categorias/${categoria.imagen}`} alt="Logo" width={1500} height={100} className="w-full h-auto" />
                                </div>
